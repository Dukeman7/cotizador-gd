import streamlit as st
import numpy as np
import pandas as pd
import plotly.graph_objects as go
from io import StringIO
import re

# --- CONFIGURACIÓN DE PÁGINA (Tema Forzado Blanco) ---
st.set_page_config(page_title="IUFO Precision Engine", layout="wide")

# Estilo CSS para forzar el fondo blanco en la app y el título
st.markdown("""
    <style>
    .main { background-color: #FFFFFF; }
    div.stTextArea textarea { font-family: monospace; }
    </style>
    """, unsafe_allow_html=True)

# --- BARRA LATERAL (CENTRO DE MANDO) ---
st.sidebar.header("🛠️ Parámetros del Algoritmo")
titulo_input = st.sidebar.text_input("Título de la Gráfica", "MATRIZ DE TARIFICACIÓN ESTRATÉGICA")

col1, col2 = st.sidebar.columns(2)
with col1:
    A = st.number_input("A", value=57.70, format="%.2f")
    C = st.number_input("C", value=2.95, format="%.2f")
    q = st.number_input("q (Max BW)", value=100400)
with col2:
    n = st.number_input("n", value=0.69, format="%.2f")
    k_log = st.number_input("k_log", value=2.10, format="%.2f")

st.sidebar.subheader("📐 Parámetros de Banda")
C_sup = st.sidebar.number_input("C_sup", value=3.20)
k_sup = st.sidebar.number_input("k_sup", value=1.05)
C_inf = st.sidebar.number_input("C_inf", value=1.00)
k_inf = st.sidebar.number_input("k_inf", value=0.35)

st.sidebar.subheader("📊 Ajustes de Gráfica")
x_max = st.sidebar.number_input("Máximo X (Mbps)", value=40000)
y_max = st.sidebar.number_input("Máximo Y ($/Mbps)", value=60)
factor_circulo = st.sidebar.slider("Tamaño de los Círculos", 1, 100, 30)

# --- CUERPO PRINCIPAL ---
st.subheader("📝 Datos del Portafolio")
raw_data = st.text_area("Pega celdas de Excel (Mbps  Precio  Clientes)", 
                        value="100  15.0  1\n1000  3.5  12\n10000  1.3  4", height=100)

def calc_curvas(bw):
    L = k_log + np.log10(bw / q)
    p_prom = (A / np.power(bw, n)) + C - L
    p_techo = p_prom + C_sup - (k_sup * L)
    p_suelo = p_prom - C_inf + (k_inf * L)
    return p_prom, p_techo, p_suelo

try:
    # Procesamiento inteligente (Regex para Tabs de Excel)
    clean_data = re.sub(r'[ \t]+', ',', raw_data.strip())
    df = pd.read_csv(StringIO(clean_data), names=['bw', 'price', 'n_clients'])
    
    # Generar Curvas
    bw_range = np.logspace(0, np.log10(x_max), 500)
    p_p, p_t, p_s = calc_curvas(bw_range)

    # --- CREAR GRÁFICA INTERACTIVA ---
    fig = go.Figure()

    # 1. Banda de Negociación
    fig.add_trace(go.Scatter(
        x=np.concatenate([bw_range, bw_range[::-1]]),
        y=np.concatenate([p_t, p_s[::-1]]),
        fill='toself',
        fillcolor='rgba(0, 68, 136, 0.05)',
        line=dict(color='rgba(255,255,255,0)'),
        hoverinfo='skip',
        name='Banda de Negociación'
    ))

    # 2. Curvas de Referencia (Sin nombre IUFO)
    fig.add_trace(go.Scatter(x=bw_range, y=p_p, name='Precio Sugerido', line=dict(color='#004488', width=2)))
    fig.add_trace(go.Scatter(x=bw_range, y=p_t, name='Límite Techo', line=dict(color='#CC0000', width=1, dash='dash')))
    fig.add_trace(go.Scatter(x=bw_range, y=p_s, name='Límite Suelo', line=dict(color='#008800', width=1, dash='dash')))

    # 3. Puntos de Clientes
    fig.add_trace(go.Scatter(
        x=df['bw'],
        y=df['price'],
        mode='markers',
        name='Clientes en el plan',
        marker=dict(
            size=df['n_clients'],
            sizemode='area',
            sizeref=2. * max(df['n_clients']) / (factor_circulo**2),
            color='#004488',
            line=dict(width=1, color='white')
        ),
        text=[f"<b>Plan: {b} Mbps</b><br>Clientes: {int(c)}<br>Precio: ${p}/Mbps" 
              for c, b, p in zip(df['n_clients'], df['bw'], df['price'])],
        hoverinfo='text'
    ))

    # --- CONFIGURACIÓN DE LAYOUT (Título Interno y Leyenda en Cuadro) ---
    fig.update_layout(
        title={
            'text': f"<b>{titulo_input}</b>",
            'y': 0.95,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
            'font': {'size': 20, 'color': '#004488'}
        },
        template='plotly_white',
        paper_bgcolor='white',
        plot_bgcolor='white',
        height=700,
        margin=dict(l=60, r=40, b=60, t=100),
        # LEYENDA AGRUPADA EN CUADRO
        legend=dict(
            orientation="v",
            yanchor="top",
            y=0.99,
            xanchor="right",
            x=0.99,
            bgcolor="rgba(255, 255, 255, 0.9)",
            bordercolor="Black",
            borderwidth=1
        ),
        hoverlabel=dict(bgcolor="white", font_size=12)
    )

    # Ejes con origen 0,0 real
    fig.update_xaxes(
        type="log", 
        title="Capacidad contratada (Mbps)", 
        range=[0, np.log10(x_max)],
        zeroline=True, zerolinecolor='black', linewidth=1.5, gridcolor='#F0F0F0'
    )
    fig.update_yaxes(
        title="Precio Unitario (USD / Mbps)", 
        range=[0, y_max],
        zeroline=True, zerolinecolor='black', linewidth=1.5, gridcolor='#F0F0F0'
    )

    st.plotly_chart(fig, use_container_width=True, theme=None)

except Exception as e:
    st.info("Pega los datos de Excel para generar el reporte de auditoría.")

st.sidebar.markdown("---")
st.sidebar.caption("Gumersinda Edition v4.3 | Luis Duque")
